...
def test_generated_names_look_like_names():
    # Names are Markov-generated; sample many and assert each is plausible:
    # capitalized, letters only, sane length, and not gibberish.
    for _ in range(200):
        for name in (identities.random_first_name(), identities.random_last_name()):
            assert re.fullmatch(r"[A-Z][a-z]+", name), name
            assert 3 <= len(name) <= 11, name
            assert identities._looks_like_name(name.lower()), name


def test_emails_are_unique():
    # Every email must be unique even though names repeat. 50k in-process.
    emails = [identities.random_identity()["email"] for _ in range(50000)]
    assert len(set(emails)) == len(emails)
    for e in emails[:50]:
        assert re.fullmatch(r"[a-z0-9._]+@[a-z.]+", e), e
