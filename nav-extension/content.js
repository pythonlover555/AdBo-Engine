 ...
 *
 * STEP 5: "email me my status?" CTA (#cidmain) -> click "Yes!", then wait 30s
 *   (it opens an email popup) before continuing.
 *
...
// Set once this page is confirmed to be an active funnel page (an anchor matched
// in main()). Gates the watchdog heartbeat below so only real funnel pages keep
// the service worker's stall clock alive.
...
// Fixed wait after clicking "Yes!" on the #cidmain CTA (it opens an email popup /
// kicks off the next step) before the extension does anything else.
const POST_YES_WAIT_MS = 30000;
...
async function selectField(id, value) {
  ...
  if (sel.value === String(value)) return; // already EXACTLY right (else force it,
  //   overwriting any value the page pre-filled with the previous lead's data)
...
async function chooseGender(gender) {
  ...
  if (hidden) setNativeValue(hidden, female ? "F" : "M"); // force the exact value
}
...
async function registrationStep() {
  ...
  // Server-supplied details. The page often arrives PRE-FILLED with the previous
  // lead's data (field-items show "valid non-empty"), so FORCE every field to our
  // values (overwrite) — otherwise the xlsx address/phone/zip never go in.
  await fillField("ad", details.address1, { overwrite: true });
  await fillField("zp", details.zip, { overwrite: true });
  await selectField("state", details.state);
  await fillField("ct", details.city, { overwrite: true });
  await fillField("tl", details.phone, { overwrite: true }); // 10 digits; field auto-formats
...
// How to handle consent / lead-gen questions (Sign Up vs No Thanks, Yes vs No
// Call, Medicare vs Skip/No Call):
//   "optIn"   -> click the POSITIVE option (Sign Up / Yes) — opt into the offer
//                (means real marketing emails/calls to the identity's contact info)
//   "decline" -> click the opt-out (No Thanks / No Call / Skip)
// Flip this one constant to switch the whole funnel's behavior.
const CONSENT_MODE = "optIn"; // "optIn" | "decline"
...
// Detected by the .sponsored tag, the "not required" / "sponsored ad" wording, or
// consent language in the disclaimer (CONSENT_RE). CONSENT_MODE decides what we do.
...
// The "opt-in" / positive answer for a consent question: a real (non-skip)
// option that ISN'T the decline. Prefer an obviously-affirmative label
// (Sign Up / Yes / I Want / Protect …); else take the first real option (for
// multi-choice consent like the Medicare ad, the top option is the consenting
// one).
const ACCEPT_RE =
  /^\s*(yes|sign\s*up|i\s*want|i'?m\s*interested|protect|stay\s*in|tell\s*me|get\s|claim|continue|i\s*agree|sure)/i;

function acceptOption(options) {
  const decline = declineOption(options);
  const positives = options.filter((o) => o !== decline && !o.isSkip);
  if (!positives.length) return null;
  return positives.find((o) => ACCEPT_RE.test(o.label)) || positives[0];
}
...
function chooseAnswer(block, qText, options) {
  ...
  // Consent / lead-gen question -> CONSENT_MODE decides.
  if (isConsentQuestion(block)) {
    if (CONSENT_MODE === "decline") {
      const decline = declineOption(options);
      if (decline) return decline;
      flag(`consent question with no decline option — "${qText.slice(0, 60)}"`);
    } else {
      const accept = acceptOption(options); // opt in: pick the positive answer
      if (accept) return accept;
      // no clear positive -> fall through and answer normally
    }
    // (fall through to ANSWER_RULES / default if no option was returned above)
  }
...
async function ctaYesStep() {
  ...
  if (cid.dataset.condNavDone === "1") return true;
  ...
  // page navigated away in the meantime, in which case the new page's script runs).
  await sleep(POST_YES_WAIT_MS);
  log("cta: 30s elapsed — continuing");
  await main();
  return true;
}

// --- STEP 6: progress-slider interstitial -> WAIT for it to disappear --
// A "step slider" (#step-slider-cont with #stepper1/2/3 — REVIEWING SURVEY /
// SPONSORED OFFERS / FINAL CHECKPOINT) is shown between funnel steps. We must
// NOT interact with it — just wait until it's gone (removed/hidden, or the page
// navigates away), then route to whatever shows next.
const SLIDER_SEL = "#step-slider-cont, .slider-stepper";

// Is any progress-slider element actually on screen (not removed/hidden)? It's
// positioned off-canvas during its animation, so we judge by display/visibility/
// opacity, NOT by on-screen position.
function sliderShown() {
  for (const el of document.querySelectorAll(SLIDER_SEL)) {
...
async function reviewingProgressStep() {
  if (!sliderShown()) return false;
  log("progress slider shown — waiting for it to disappear…");
  const gone = await waitForCondition(() => (sliderShown() ? null : true), 120000);
  if (!gone) {
    flag("progress slider still showing after 120s");
    return true;
  }
  log("progress slider gone — continuing");
  // If it was an in-place transition (no navigation), handle whatever is now
  // shown. If it navigated, this context is gone and the new page's script runs.
  await main();
  return true;
}
...
async function main() {
  ...
  if (document.querySelector("#step-slider-cont, .slider-stepper")) {
    return void (await reviewingProgressStep());
  }
...
// Watchdog heartbeat: once main() confirms this is an active funnel page, ping
// the service worker every ~10s. This lets its stall watchdog tell a long-but-
// healthy funnel page (an 80-question survey, the 30s post-"Yes!" wait, the 120s
// slider) apart from a genuinely hung one. Pages with no funnel anchor never set
// heartbeatActive, so they correctly read as "no progress" and get restarted.
